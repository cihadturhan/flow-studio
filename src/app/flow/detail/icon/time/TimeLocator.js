...
(function (angular) {
    ...
    angular
        .module('app.flow')
        .service('TimeLocator', TimeLocatorService);

    TimeLocatorService.$inject = ['CANVAS'];

    /* @ngInject */
    function TimeLocatorService(CANVAS) {
        var vm = this;
        ...
        function create(){
            var locator = draw2d.layout.locator.ConnectionLocator.extend({
                NAME: CANVAS.basic.locator.time_locator.config.name,
                relocate: relocate,
                rotate: rotate
            });
            ...
            function relocate(index, target) {
                ...
                var rotAnchor = this.rotate(length / 4 - target.getWidth() / 2, distance, 0, 0, radian);
